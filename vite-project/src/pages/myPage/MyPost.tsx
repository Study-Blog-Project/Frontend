...
function MyPost(props: { isAllPost: boolean }) {
  ...
  useEffect(() => {
    console.log("role:", props.isAllPost);
    if (props.isAllPost === false) {
  ...
  }, [selectedRecruitTabContent, selectedCategoryTabContent, props.isAllPost]);
  ...
  return (
    <>
      ...
      <div className="w-full flex flex-col">{boardResponse && PostDivs(boardResponse)}</div>
    </>
  );
}
